...
namespace minidom
{
    public partial class WebSite
    {
        ...
        /// <remarks></remarks>
            ...
            {
                m_SessionID = reader.Read("Session", ref m_SessionID);
                m_UserID = reader.Read("UserID", ref m_UserID);
                m_UserName = reader.Read("UserName", ref m_UserName);
                m_Data = reader.Read("Data", ref m_Data);
                m_Secure = reader.Read("Secure", ref m_Secure);
                m_Protocol = reader.Read("Protocol", ref m_Protocol);
                m_SiteName = reader.Read("SiteName", ref m_SiteName);
                m_PageName = reader.Read("ScriptName", ref m_PageName);
                m_QueryString = reader.Read("QueryString", ref m_QueryString);
                m_PostedData = reader.Read("PostedData", ref m_PostedData);
                m_Referrer = reader.Read("Referrer1", ref m_Referrer);
                if (string.IsNullOrEmpty(m_Referrer))
                    m_Referrer = reader.Read("Referrer", ref m_Referrer);
                m_ExecTime = reader.Read("ExecTime", ref m_ExecTime);
                m_StatusDescription = reader.Read("PageStatus", ref m_StatusDescription);
                m_StatusCode = reader.Read("PageStatusCode", ref m_StatusCode);
                m_IDAnnuncio = reader.Read("IDAnnunction", ref m_IDAnnuncio);
