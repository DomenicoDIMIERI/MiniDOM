...
namespace minidom
{
    ...
    /// </summary>
        ...
        [Serializable]
        public class CCollegamento : Databases.DBObject, IComparable<CCollegamento>, IComparable
        {
            ...
            private CSubCollegamenti m_Childs;
            private string m_CallModule;
            private string m_CallAction;
            private LinkUserAllowNegateCollection m_UsersAuth;
            private LinkGroupAllowNegateCollection m_GroupAuth;
        ...
        /// <remarks></remarks>
            ...
            {
                m_Nome = reader.Read("Nome", ref m_Nome);
                m_Descrizione = reader.Read("Descrizione", ref m_Descrizione);
                m_Link = reader.Read("Link", ref m_Link);
                m_Gruppo = reader.Read("Gruppo", ref m_Gruppo);
                m_Target = reader.Read("Target", ref m_Target);
                m_IconURL = reader.Read("IconURL", ref m_IconURL);
                m_EncriptURL = reader.Read("EncriptURL", ref m_EncriptURL);
                m_IDParent = reader.Read("IDParent", ref m_IDParent);
                m_CallModule = reader.Read("CallModule", ref m_CallModule);
                m_CallAction = reader.Read("CallAction", ref m_CallAction);
                m_Attivo = reader.Read("Attivo", ref m_Attivo);
                m_Posizione = reader.Read("Posizione", ref m_Posizione);
                m_Flags = reader.Read("Flags", ref m_Flags);
                m_PostedData = reader.Read("PostedData", ref m_PostedData);
            ...
            protected override void SetFieldInternal(string fieldName, object fieldValue)
            {
                switch (fieldName ?? "")
                {
                    case var @case when CultureInfo.CurrentCulture.CompareInfo.Compare(@case, "Nome", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                    ...
                    case var case2 when CultureInfo.CurrentCulture.CompareInfo.Compare(case2, "Link", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_Link = DMD.XML.Utils.Serializer.DeserializeString(fieldValue);
                            break;
                        }

                    case var case3 when CultureInfo.CurrentCulture.CompareInfo.Compare(case3, "Gruppo", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_Gruppo = DMD.XML.Utils.Serializer.DeserializeString(fieldValue);
                            break;
                        }

                    case var case4 when CultureInfo.CurrentCulture.CompareInfo.Compare(case4, "Target", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_Target = DMD.XML.Utils.Serializer.DeserializeString(fieldValue);
                            break;
                        }

                    case var case5 when CultureInfo.CurrentCulture.CompareInfo.Compare(case5, "IconURL", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_IconURL = DMD.XML.Utils.Serializer.DeserializeString(fieldValue);
                            break;
                        }

                    case var case6 when CultureInfo.CurrentCulture.CompareInfo.Compare(case6, "EncriptURL", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_EncriptURL = DMD.XML.Utils.Serializer.DeserializeBoolean(fieldValue) == true;
                            break;
                        }

                    case var case7 when CultureInfo.CurrentCulture.CompareInfo.Compare(case7, "IDParent", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_IDParent = (int)DMD.XML.Utils.Serializer.DeserializeInteger(fieldValue);
                            break;
                        }

                    case var case8 when CultureInfo.CurrentCulture.CompareInfo.Compare(case8, "CallModule", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_CallModule = DMD.XML.Utils.Serializer.DeserializeString(fieldValue);
                            break;
                        }

                    case var case9 when CultureInfo.CurrentCulture.CompareInfo.Compare(case9, "CallAction", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_CallAction = DMD.XML.Utils.Serializer.DeserializeString(fieldValue);
                            break;
                        }

                    case var case10 when CultureInfo.CurrentCulture.CompareInfo.Compare(case10, "Attivo", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_Attivo = DMD.XML.Utils.Serializer.DeserializeBoolean(fieldValue) == true;
                            break;
                        }

                    case var case11 when CultureInfo.CurrentCulture.CompareInfo.Compare(case11, "Posizione", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_Posizione = (int)DMD.XML.Utils.Serializer.DeserializeInteger(fieldValue);
                            break;
                        }

                    case var case12 when CultureInfo.CurrentCulture.CompareInfo.Compare(case12, "Flags", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            m_Flags = (CollegamentoFlags)DMD.XML.Utils.Serializer.DeserializeInteger(fieldValue);
                            break;
                        }

                    case var case13 when CultureInfo.CurrentCulture.CompareInfo.Compare(case13, "PostedData", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
        ...
        /// <remarks></remarks>
            ...
            public virtual int CompareTo(CCollegamento other)
            {
                ...
                if (ret == 0)
                    ret = DMD.Arrays.Compare(ref m_Posizione, ref other.m_Posizione);
                if (ret == 0)
                    ret = DMD.Strings.Compare(m_Descrizione, other.m_Descrizione, true);
                return ret;
            }
