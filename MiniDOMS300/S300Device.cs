...
{
    /// <summary>
    /// Tipi di marcature da recuperare <see cref="S300Device"/>
    /// </summary>
    ...
    /// <summary>
    /// Rappresenta un device di tipo ANVIZ S300
    /// </summary>
    [Serializable]
    public class S300Device
    {
        ...
        {
            int comPort = DMD.Integers.ValueOf(Strings.Mid(m_Address, 4));
        ...
        private string ToAnsiString(byte[] arr)
        {
            ...
            int p = Strings.InStr(ret, DMD.Strings.vbNullChar);
            if (p > 0)
        ...
        public void SetConfiguration(S300Config value)
        {
            EnsureConnected();
            int ret;
            ret = CKT_DLL.CKT_SetRingAllow(m_IDNumber, value.RingAllow? 1 : 0);
            if (ret == 0)
                throw new Exception("Errore in CKT_SetRingAllow: " + Marshal.GetLastWin32Error());

            ret = CKT_DLL.CKT_SetRealtimeMode(m_IDNumber, value.RealtimeMode? 1 : 0);
            if (ret == 0)
                throw new Exception("Errore in CKT_SetRealtimeMode: " + Marshal.GetLastWin32Error());

            ret = CKT_DLL.CKT_SetAutoUpdate(m_IDNumber, (value.AutoUpdateFingerprint)? 1 : 0);
            if (ret == 0)
        ...
        {
            EnsureConnected();
            var arr = DMD.Arrays.Empty<S300Clocking>();
            int cnt = 0;
            ...
            {
                arr = DMD.Arrays.Empty<S300Clocking>();
            }
        ...
        public CKT_DLL.RINGTIME[] GetRingTimes()
        {
            ...
            int ret = CKT_DLL.CKT_GetHitRingInfo(DeviceID, out tarr);
            if (ret == 0)
                throw new S300Exception();
