...
namespace minidom
{
    ...
    {
        [Serializable]
        public class IPADDRESSinfo : Databases.DBObject, IComparable
        {
            private byte[] m_IP;
            ...
            public IPADDRESSinfo(string value)
            {
                ...
                {
                    m_NetMask = GetMaskBytes32(DMD.Integers.CInt(netMask));
                }
            ...
            public bool Match(string ip)
            {
                ...
                for (int i = 0, loopTo = Math.Min(DMD.Arrays.UBound(m_IP), DMD.Arrays.UBound(m_NetMask)); i <= loopTo; i++)
                {
                ...
                else
                {
                    ...
                    if (DMD.Arrays.UBound(items) == 3)
                    {
                        ...
                        for (int i = 0, loopTo = DMD.Arrays.UBound(items); i <= loopTo; i++)
                            bytes[i] = Conversions.ToByte(items[i]);
                    }
                    else
                    {
            ...
            private byte GetHighBits(int bits)
            {
                ...
                {
                    bits -= 1;
                    ret = (int)((long)ret | (long)Math.Pow(2d, bits));
                }
            ...
            private byte[] FromString(string str)
            {
                ...
                for (int i = 0, loopTo = DMD.Arrays.UBound(tmp); i <= loopTo; i++)
                    ret[i] = Conversions.ToByte(tmp[i]);
                return ret;
            }
        ...
        /// <remarks></remarks>
            ...
            {
                IP = FromString(reader.GetValue("IP", DMD.Strings.vbNullString));
                NetMask = FromString(reader.GetValue("NetMask", DMD.Strings.vbNullString));
                m_Descrizione = reader.Read("Descrizione", ref m_Descrizione);
                m_Allow = reader.Read("Allow", ref m_Allow);
                m_Negate = reader.Read("Negate", ref m_Negate);
                m_Interno = reader.Read("Interno", ref m_Interno);
                m_AssociaUfficio = reader.Read("AssociaUfficio", ref m_AssociaUfficio);
            ...
            public int CompareTo(IPADDRESSinfo obj)
            {
                ...
                {
                    return DMD.Strings.Compare(ToString(), obj.ToString());
                }
            }
