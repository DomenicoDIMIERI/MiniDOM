...
//            : Databases.DBObjectBase
        ...
        /// <remarks></remarks>
            ...
            //    m_SiteDescription = reader.Read("SiteDescription", this.m_SiteDescription);
            //    m_SiteURL = reader.Read("SiteURL", this.m_SiteURL);
            //    m_InfoEMail = reader.Read("InfoEMail", this.m_InfoEMail);
            ...
            //    m_LogoURL = reader.Read("LogoURL", this.m_LogoURL);
            //    m_Note = reader.Read("Note", this.m_Note);
            //    m_PublicURL = reader.Read("PublicFolder", this.m_PublicURL);
            //    // m_ExternalDB = d bRis("ExternalDB") 
            //    m_SupportEMail = reader.Read("SupportEMail", this.m_SupportEMail);
            //    m_SimboloValuta = reader.Read("SimboloValuta", this.m_SimboloValuta);
            //    m_DecimaliPerValuta = reader.Read("DecimaliPerValuta", this.m_DecimaliPerValuta);
            //    m_DecimaliPerPercentuale = reader.Read("DecimaliPerPercentuale", this.m_DecimaliPerPercentuale);
            //    m_Flags = reader.Read("Flags", this.m_Flags);
            //    m_UploadSpeedLimit = reader.Read("UploadSpeedLimit", this.m_UploadSpeedLimit);
            //    m_NumberOfUploadsLimit = reader.Read("NumberOfUploadsLimit", this.m_NumberOfUploadsLimit);
            //    m_UploadTimeOut = reader.Read("UploadTimeOut", this.m_UploadTimeOut);
            //    m_ShortTimeOut = reader.Read("ShortTimeOut", this.m_ShortTimeOut);
            //    m_LongTimeOut = reader.Read("LongTimeOut", this.m_LongTimeOut);
            //    m_KeyWords = reader.Read("KeyWords", this.m_KeyWords);
            //    m_UploadBufferSize = reader.Read("UploadBufferSize", this.m_UploadBufferSize);
            //    m_CRMMaxCacheSize = reader.Read("CRMMaxCacheSize", this.m_CRMMaxCacheSize);
            //    m_CRMUnloadFactor = reader.Read("CRMUnloadFactor", this.m_CRMUnloadFactor);
            //    m_DeleteLogFilesAfterNDays = reader.Read("DelLogNDays", this.m_DeleteLogFilesAfterNDays);
            ...
            public static SiteConfig Load(string fileName)
            {
                ...
                {
                    return (SiteConfig)ser.Deserialize(reader);
                }
            }

            public static void Save(SiteConfig config, string fileName)
            {
                if (config is null)
                ...
                {
                    String backupFileName = IOUtils.AppendToFileName(fileName, "(" + DateUtils.GetTimeStampForFile() + ")");
            ...
            protected void DoChanged(string propName, object newVal = null, object oldVal = null)
            {
                //base.DoChanged(propName, newVal, oldVal);
                Instance.doConfigChanged(new EventArgs());
            }

            void DMD.XML.IDMDXMLSerializable.SetFieldInternal(string fieldName, object fieldValue) { this.SetFieldInternal(fieldName, fieldValue);  }
            void DMD.XML.IDMDXMLSerializable.XMLSerialize(XMLWriter writer) { this.XMLSerialize(writer); }

            public void Save()
            {
               ((IWebApplicationContext)  WebSite.Instance.AppContext).SaveConfiguration(this);
            }
        }
    }
}