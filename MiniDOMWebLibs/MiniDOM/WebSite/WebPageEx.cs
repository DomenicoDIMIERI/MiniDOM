...
namespace minidom
{
    public partial class WebSite
    {
            ...
            : Page
        {
            ...
            public const int DELAYTONOTIFY = 5000;

            private static Dictionary<string, reqInfo> pendingRequests = new  Dictionary<string, reqInfo>();
            ...
            public sealed class reqInfo
            {
                ...
                {
                    Interlocked.Increment(ref __ids);
                    this.reqID = __ids;
                ...
                public string pageUrl
                {
                    get
                    {
                        return (this.page is null)? string.Empty : this.page.ToString(); //.Request.Url.ToString()
                    }
                }

                /// <summary>
                /// Restituisce una stringa univoca per la pagina
                /// </summary>
                public string Key
                {
                    get
                    {
                        return string.Format("K{0}", this.reqID);
                    }
                }
                ...
                /// <summary>
                /// Notifica la fine dell'elaborazione
                /// </summary>
                ...
                public int ExecTime
                {
                    get
                    {
                        if (IsRunning())
                        {
                            return (int)Maths.Floor((DMD.DateUtils.Now() - StartTime).TotalMilliseconds);
                        }
                        else
                        {
                            return (int)Maths.Floor((EndTime.Value - StartTime).TotalMilliseconds);
                        }
                    }
                }
            }
            ...
            /// <summary>
            /// Restituisce la collezione delle richieste pendenti
            /// </summary>
            /// <returns></returns>
            public static IEnumerable<reqInfo> GetPendingRequests()
            {
            ...
            {
                 var alerts = new List<reqInfo>();
                 lock (pendingRequests)
                 {
                    var keys = new List<string>(pendingRequests.Keys);
                ...
                foreach (var req in alerts)
                {
                    var buff = new System.Text.StringBuilder();
                    buff.Append(Sistema.Formats.FormatUserDateTime(DMD.DateUtils.Now()));
                    buff.Append(" - "); buff.Append(req.UserName);
                    buff.Append(" - Timeout dello script: ");
                    buff.Append(req.pageUrl);
                    buff.Append(" (");
                    buff.Append(req.Timeout);
                    buff.Append(")"); 
                    Sistema.ApplicationContext.Log(buff.ToString());
                }
            }
        ...
        /// </summary>
            public WebPageEx()
            {
                ...
                lock (pendingRequests)
                {
                    if (_timer is null) _timer = InitTimer();

                    req = new reqInfo(this, GetDefaultTimeOut() * 1000);
                    pendingRequests.Add(req.Key, req);
                }
        ...
        /// <remarks></remarks>
            public virtual Sistema.CModule CurrentModule
            {
                get
                {
                    ...
                    {
                        string m = this.n2str("_m", "");
                        if (DMD.Strings.IsNumeric(m))
                        {
                            m_CurrentModule = Sistema.Modules.GetItemById(DMD.Integers.ValueOf(m));
                        }
                        else
                        {
            ...
            /// <summary>
            /// Restituisce il nome dell'azione richiesta
            /// </summary>
            public virtual string RequestedAction
            {
                get
                {
                    if (string.IsNullOrEmpty(m_a))
                        m_a = DMD.Strings.Trim(this.n2str("_a", ""));
                    return m_a;
                }
            }
            ...
            public virtual bool IsAllowedIP(string value)
            {
                ...
                else if (CultureInfo.CurrentCulture.CompareInfo.Compare(lastUserIP ?? "", value ?? "", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0)
                {
                    ...
                    ret = DMD.Booleans.ValueOf(Session["LastUserIP_Res"]);
                }
        ...
        /// <remarks></remarks>
            ...
            /// <summary>
            /// Restituisce vero se l'IP è negato
            /// </summary>
            /// <param name="value"></param>
            /// <returns></returns>
        ...
        /// <remarks></remarks>
            ...
            /// <summary>
            /// Genera il codice html della pagina
            /// </summary>
            ...
            public T? GetParameter1<T>(string paramName, object defValue = null) where T : struct
            {
                if (!IsParameterSet(paramName))
                    return (T?)defValue;
                var tp = typeof(T);
                string param = this.n2str(paramName, "");
                object ret = null;
            ...
            public string GetParameter1(string paramName, string defValue)
            {
            ...
            public string n2str(string paramName, string defValue = "")
            {
                string ret = null;
            ...
            public int? n2int(string paramName, int? defValue = default)
            {
                string ret = this.n2str(paramName, null);
                return DMD.Integers.TryParse(ret, defValue);
            }

            public DateTime? n2date(string paramName, DateTime? defValue = default)
            {
                string ret = this.n2str(paramName, null);
                return DMD.DateUtils.TryParse(ret, defValue);
            }

            public double? n2float(string paramName, double? defValue = default)
            {
                string ret = this.n2str(paramName, null);
                return DMD.Doubles.TryParse(ret, defValue);
            }

            public bool? n2bool(string paramName, bool? defValue = default)
            {
                string ret = this.n2str(paramName, null);
                return DMD.Booleans.TryParse(ret, defValue);
            }
