...
namespace minidom
{
    ...
    {
        public class CSessionInfo : XML.IDMDXMLSerializable, IDisposable
        {
            ...
            public Sistema.CUser OriginalUser;
            public CKeyCollection Parameters;
            public string Descrizione;
            public CKeyCollection<Databases.DBObjectCursorBase> RemoteOpenedCursors = new CKeyCollection<Databases.DBObjectCursorBase>();
            ...
            public void Reset()
            {
                lock (@lock)
                {
                    foreach (Databases.DBObjectCursorBase c in RemoteOpenedCursors)
                    {
                        ...
                        {
                            Sistema.ApplicationContext.Log("Session: " + SessionID + " resetting cursor " + Information.TypeName(c) + " error" + DMD.Strings.vbCrLf + ex.Message + DMD.Strings.vbCrLf + ex.StackTrace);
                        }
                    }
            ...
            public void SetFieldInternal(string fieldName, object fieldValue)
            {
                switch (fieldName ?? "")
                {
                    ...
                    case var case1 when CultureInfo.CurrentCulture.CompareInfo.Compare(case1, "SessionID", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            SessionID = DMD.XML.Utils.Serializer.DeserializeString(fieldValue);
                            break;
                        }

                    case var case2 when CultureInfo.CurrentCulture.CompareInfo.Compare(case2, "RemoteIP", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            RemoteIP = DMD.XML.Utils.Serializer.DeserializeString(fieldValue);
                            break;
                        }

                    case var case3 when CultureInfo.CurrentCulture.CompareInfo.Compare(case3, "RemotePort", CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreWidth) == 0:
                        {
                            RemotePort = DMD.Strings.CStr(XML.Utils.Serializer.DeserializeInteger(fieldValue));
                            break;
                        }
            ...
            public void XMLSerialize(XML.XMLWriter writer)
            {
